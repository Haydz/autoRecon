...
def Enum4Linux(ipToScan):
	if constants.osVersion == 'Debian':
		...
		command = '%s/enum4linux.py %s >> enum4_%s.ouput' %(Enum4LinuxPath, ipToScan, ipToScan)
...
def eyewitness(filename, outputName): #expecting IP addrees list
	...
	if constants.osVersion == 'Debien':
		eyewitnessPath = '/pentest/intelligence-gathering/eyewitness'
		command = '%s/Eyewitness.py --headless--prepend-https --no-prompt  -x %s -d %s' %(eyewitnessPath,filename, outputName)
	if constants.osVersion == 'Kali':
